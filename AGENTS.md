...
`turbo-themes` is a multi-language theming library (design tokens →
CSS/TS/Python/Ruby/Swift artifacts) plus an Astro documentation/demo site. The primary
developer surface is the JS/TS workspace driven by **Bun**; Python (via `uv`) is only
used for linting, and Ruby/Swift are optional platform packages.

Standard commands live in `package.json` scripts and `CONTRIBUTING.md` — use those as
the source of truth. Notes below are non-obvious caveats for this environment.
...
- Ruby and Swift are **not** installed; the gem (`bundle`/`rake`) and Swift Package
  targets cannot be built/tested here without installing those toolchains. This does not
  affect the JS/TS product or docs site.

### Running / testing

- Dev server (docs + live theme selector): `cd apps/site && bun run dev` →
  http://localhost:4321/ . Run `bun run build` at the repo root first so the workspace
  packages the site imports are built.
- Unit tests: `bun run test` (Vitest, ~2200 tests with coverage).
- Lint: `bun run lint` (auto-creates its `uv` environment on first run, so the first
...
- The Playwright browser binary (chromium headless shell) is installed via

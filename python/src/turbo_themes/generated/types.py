...
class Components:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Components":
        ...
        return cls(
            ...
            message=(
                MessageComponent.from_dict(data["message"])
                if "message" in data
                else None
            ),
            ...
            notification=(
                NotificationComponent.from_dict(data["notification"])
                if "notification" in data
                else None
            ),
            modal=ModalComponent.from_dict(data["modal"]) if "modal" in data else None,
            dropdown=(
                DropdownComponent.from_dict(data["dropdown"])
                if "dropdown" in data
                else None
            ),
...
class Tokens:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Tokens":
        ...
        return cls(
            ...
            elevation=(
                Elevation.from_dict(data["elevation"]) if "elevation" in data else None
            ),
            animation=(
                Animation.from_dict(data["animation"]) if "animation" in data else None
            ),
            opacity=Opacity.from_dict(data["opacity"]) if "opacity" in data else None,
            components=(
                Components.from_dict(data["components"])
